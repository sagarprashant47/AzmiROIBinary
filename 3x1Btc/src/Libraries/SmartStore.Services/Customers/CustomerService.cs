...
namespace SmartStore.Services.Customers
{
	public partial class CustomerService : ICustomerService
	{
		...
		public Plan GetCurrentFullPlan(int CustomerId)
		{
			var customer = GetCustomerById(CustomerId);
			if (customer == null)
				return null;
			...
			var Id = _customerPlanRepository.Table.Where(e => e.CustomerId == CustomerId).Select(e => e.PlanId).FirstOrDefault();
			var Name = _planRepository.Table.Where(e => e.Id == Id).FirstOrDefault();
			return Name;
		}
		...
		public virtual float GetAvailableBalance(int customerid)
		{
			...
			totalTradeFunding = totalTradeFunding > 0 ? (totalTradeFunding * 60 / 100) : 0;
			var availablebalance = totalFunding + totalTotalEarning + totalTradeFunding - (totalWithdrawal + totalTransfer);
			return availablebalance;
		}

		public virtual float GetTokenBalance(int customerid)
		{
			var customer = GetCustomerById(customerid);
			if (customer == null)
				return 0;
			...
			var tokenBalance = totalTradeFunding > 0 ? (totalTradeFunding * 40 / 100) : 0; ;
			return tokenBalance;
		}
